...
void VulkanEngine::init_default_renderpass()
{
	...
	// connector the color attachment to the info
...
void VulkanEngine::draw()
{
	...
	// finalize the command buffer (we can no longer add commands, but it can now be
	// executed)
	VK_CHECK(vkEndCommandBuffer(cmd));

	// prepare the submission to the queue
	// we want to wait on the _presentSemaphore, as that semaphore is signaled when
	// the swapchain is ready
	// we will signal the _renderSemaphore, to signal the rendering has finished
...
bool VulkanEngine::load_shader_module(const char* filePath, VkShaderModule* outShaderModule)
{
	...
	// spirv expects the buffer to be on uint32, so make sure to reserve a int vector
...
void VulkanEngine::init_pipelines()
{
	...
	// we are not using descriptor sets or other systems yet, so no need to use anything
	// other than empty default
...
VkPipeline PipelineBuilder::build_pipeline(VkDevice device, VkRenderPass pass)
{
	...
	// it's easy to error out on create graphics pipeline, so we handle it a bit better
	// than the common VK_CHECK case
